...
class EurekaReward():
    ...
    def compute_reward(self):
        ...
        pair1_sync = contact_states[:, 0] - contact_states[:, 1]  # Should alternate (closer to 0 over time)
        pair2_sync = contact_states[:, 2] - contact_states[:, 3]  # Should alternate (closer to 0 over time)
        ...
        # When front legs on the ground, want the rear legs in the air
        sum_front = contact_states[:, 0] + contact_states[:, 1]  # front-left + front-right
        sum_rear = contact_states[:, 2] + contact_states[:, 3]  # rear-left + rear-right
        # A simple measure of how "in phase" they are:
        sum_diff = sum_front - sum_rear
        ...
        action_rate_reward = -0.01 * torch.sum(torch.square(env.last_actions - env.actions), dim=1)
        ...
        total_reward = (tracking_lin_vel_reward + tracking_ang_vel_reward + lin_vel_z_reward + ang_vel_xy_reward +
                        torques_reward + dof_acc_reward + feet_air_time_reward + collision_reward +
