...
from repo.unitree_rl_gym.legged_gym.utils import get_args, export_policy_as_jit, task_registry_pref, Logger
import torch
import cv2
...
task_registry_pref.register('gpt_go2', LLMPrefGo2RobotFlat, LLMPrefGo2RobotFlatCfg, LLMPrefGo2RobotFlatCfgPPO)

def play(args):
    env_cfg, train_cfg = task_registry_pref.get_cfgs(name=args.task)
    ...
    # prepare environment
    env, _ = task_registry_pref.make_env(name=args.task, args=args, env_cfg=env_cfg)

    # print(env.commands)
    obs = env.get_observations()
    # load policy
    train_cfg.runner.resume = True
    ppo_runner, train_cfg = task_registry_pref.make_alg_runner(env=env, name=args.task, args=args, train_cfg=train_cfg, log_root=args.log_root)
    ...
    out_fixed = cv2.VideoWriter(args.log_root + '/' + args.load_run + '/' + args.test_direct + f'_fixed{args.checkpoint}.mp4', fourcc, 60.0, (camera_props.width, camera_props.height))
    print("save fixed video to:")
    print(args.log_root + '/' + args.load_run + '/' + args.test_direct + f'fixed{args.checkpoint}.mp4')
    ...
    out_dynamic = cv2.VideoWriter(args.log_root + '/' + args.load_run + '/' + args.test_direct + f'_right{args.checkpoint}.mp4', fourcc, 60.0, (camera_props.width, camera_props.height))
