...
from repo.unitree_rl_gym.legged_gym.utils import get_args, task_registry_wave_pref
...
def main():
    ...
    env_cfg = LLMPrefGo2WaveCfg()
    env_cfg.env.reward_module_name = args.reward_module_name

    env_cfg.terrain.mesh_type = 'trimesh'
    env_cfg.terrain.selected = True
    env_cfg.terrain.curriculum = False
    env_cfg.terrain.terrain_type = 'curriculum_wave'
    env_cfg.terrain.num_rows = 20
    env_cfg.terrain.num_cols = 5

    task_registry_wave_pref.register(args.task, LLMPrefGo2RobotWave, env_cfg, LLMPrefGo2WaveCfgPPO)

    env, env_cfg = task_registry_wave_pref.make_env(name=args.task, args=args)

    ppo_runner, train_cfg = task_registry_wave_pref.make_alg_runner(
