...
from .llmpref_go2_robot_stairs_config import LLMPrefGo2RobotStairsCfg
...
# Modified from LLMHydra2Go2Robot and add buffers for preference learning

class LLMPrefGo2RobotStairs(BaseTask):
    def __init__(self, cfg: LLMPrefGo2RobotStairsCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.difficulty_levels = torch.full((self.num_envs,), self.cfg.env.init_difficulty_level, dtype=torch.long, device=sim_device)
    ...
    def post_physics_step(self):
        ...
        # PJ: also add this to refresh the body state each step
    ...
    def check_termination(self):
        ...
        # I keep it there for reward calculation. But I will comment out the check_termination in the post_physics_step to prevent the episode to be stopped
    ...
    def _get_feet_heights(self) -> torch.Tensor:
        """
        Returns the current heights (z-coordinates in world frame) of the robot's four feet
        for all environments.

        Shape: (num_envs, 4)
        """
        # 1) Refresh rigid body state to get latest data
        self.gym.refresh_rigid_body_state_tensor(self.sim)

        # 2) Extract positions of the four feet from self.rigid_body_states
        #    shape = (num_envs, 4, 3) for x, y, z
        feet_positions = self.rigid_body_states[:, self.feet_indices, 0:3]

        # 3) Slice out just the z-coordinate
        feet_heights = feet_positions[..., 2]  # shape: (num_envs, 4)
    ...
    def _init_buffers(self):
        ...
        # PJ: Acquire rigid body states. add for getting the feet height
        rigid_body_state_tensor = self.gym.acquire_rigid_body_state_tensor(self.sim)

        self.gym.refresh_dof_state_tensor(self.sim)
        self.gym.refresh_actor_root_state_tensor(self.sim)
        self.gym.refresh_net_contact_force_tensor(self.sim)
        # PJ: Refresh the rigid-body state as well
        self.gym.refresh_rigid_body_state_tensor(self.sim)
        ...
        # PJ: wrap the rigid-body states
        # shape: (num_envs * num_bodies, 13) -> reshape -> (num_envs, num_bodies, 13)
        # In Isaac Gym, each rigid body’s state is stored as a 13-element vector
        # [x, y, z,   quat_w, quat_x, quat_y, quat_z,   vel_x, vel_y, vel_z,   ang_vel_x, ang_vel_y, ang_vel_z]
        ...
        self.base_pos_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, len(self.base_pos[1]), device=self.device, dtype=torch.float, requires_grad=False)
        self.rpy_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, len(self.rpy[1]), device=self.device, dtype=torch.float, requires_grad=False)
        self.base_lin_vel_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, len(self.base_lin_vel[1]), device=self.device, dtype=torch.float, requires_grad=False)
        self.base_ang_vel_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, len(self.base_ang_vel[1]), device=self.device, dtype=torch.float, requires_grad=False)
        self.feet_contacts_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, len(self.feet_indices), dtype=torch.bool, device=self.device, requires_grad=False)
        self.commands_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, self.cfg.commands.num_commands, device=self.device, dtype=torch.float, requires_grad=False)
        self.actions_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, self.num_actions, device=self.device, dtype=torch.float, requires_grad=False)
        ...
        self.obs_pairs_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, len(self.obs_buf[1]), device=self.device, dtype=torch.float, requires_grad=False)
        ...
        self.feet_heights_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, len(self.feet_indices), dtype=torch.float, device=self.device, requires_grad=False)
        ...
        self.ground_height_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, device=self.device, dtype=torch.float, requires_grad=False)
