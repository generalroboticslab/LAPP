...
class LLMPrefGo2RobotFlatCfg(BaseConfig):
    class env:
        ...
        test = False
        reward_module_name = 'flat_pref_prompt.reward.go2_forward_reward'  # Default reward module
    ...
    class commands:
        ...
        class ranges:
            lin_vel_x = [-0.0, 2.2]  # min max [m/s]  [-1.0, 2.2] [-0.0, 2.2] [1.5-0.0, 1.5+0.0] for stairs: [-0.0, 1.1]
            lin_vel_y = [-0.0, 0.0]  # min max [m/s]  [-1.0, 1.0] for stairs: [-0.0, 0.0]
            ang_vel_yaw = [-0.0, 0.0]  # min max [rad/s]  [-1.0, 1.0] for stairs: [-0.0, 0.0]
            heading = [-0.0, 0.0]  # min max [rad]  [-3.14, 3.14] for stairs: [-0.0, 0.0]

    class init_state:
        pos = [0.0, 0.0, 0.34] # x,y,z [m]  [0.0, 0.0, 0.42]
...
class LLMPrefGo2RobotFlatCfgPPO(BaseConfig):
    seed = 1
    ...
    class runner:
        ...
        # logging
        save_interval = 100  # 50 check for potential saves every this many iterations  50
        experiment_name = 'gpt_go2'
        run_name = ''
        # load and resume
        resume = False
