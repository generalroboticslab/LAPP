...
from repo.unitree_rl_gym.legged_gym import LEGGED_GYM_ROOT_DIR, envs
...
from .pure_go2_robot_config import PureGo2Cfg
from .terrain2 import Terrain2


class LLMPrefGo2RobotObstacles(BaseTask):
    def __init__(self, cfg: PureGo2Cfg, sim_params, physics_engine, sim_device, headless):
    ...
    def _create_terrain(self):
        ...
        # Create the terrain
        self.terrain = Terrain2(self.cfg.terrain, num_robots=self.num_envs)
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.cfg.terrain.terrain_type == 'pyramid_stairs' or self.cfg.terrain.terrain_type == 'curriculum_slope' \
                or self.cfg.terrain.terrain_type == 'curriculum_wave' or self.cfg.terrain.terrain_type == 'curriculum_obs':
            # Adjust the z position based on terrain height
            x = self.root_states[env_ids, 0]
            y = self.root_states[env_ids, 1]
            z = self._get_terrain_heights(x, y)
    ...
    # =============================================
    def _get_terrain_heights(self, x, y):
        ...
        heights = np.array([heights]) if not isinstance(heights, np.ndarray) else heights
    ...
    # =============================================
    def xw_update_diff_levels(self, score):
        '''
        This is Xiao's implementation of updating the difficulty level.
        '''
        threshold = self.cfg.terrain.upgrade_threshold
        min_level = 0
        max_level = len(self.cfg.terrain.difficulty_levels) - 1

        prev_level = self.difficulty_levels[0].item()
        # threshold -= 5 * prev_level
        new_level = prev_level

        # threshold = 40

        if score > threshold:
            new_level = min(prev_level + 1, max_level)
            self.difficulty_levels[:] = new_level

        if new_level != prev_level:
            print("@@@")
            print(f"Difficulty level updated from {prev_level} to {new_level}")
            print("@@@")
    ...
    def _init_buffers(self):
        ...
        self.base_ang_vel_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env,
                                            ...
                                            requires_grad=False)
        self.feet_contacts_buf = torch.zeros(self.num_pairs, 2, self.cfg.env.num_steps_per_env, len(self.feet_indices),
    ...
    def _prepare_reward_function(self):
        ...
        self.reward_container = reward_module.CustomReward(self)
