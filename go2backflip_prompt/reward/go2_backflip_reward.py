...
class EurekaReward():
    ...
    def compute_reward(self):
        ...
        # Reward for angular velocity around y-axis (pitch), encourage backflip rotation
        pitch_rate = env.base_ang_vel[:, 1]  # Angular velocity around y-axis (pitch)
        # Clamp the pitch rate to a minimum of -7 rad/s (maximum negative rotation speed)
        ...
        # pitch_rate_reward = 1.0 * (-clamped_pitch_rate)  # Reward negative pitch rates

        # Reward for pitch backwards for the back flip
        pitch = env.rpy[:, 1]
        last_pitch = env.last_rpy[:, 1]
        delta_pitch = (pitch - last_pitch + torch.pi) % (2 * torch.pi) - torch.pi
        delta_pitch_reward = torch.where(delta_pitch < 0, -delta_pitch, torch.zeros_like(delta_pitch))
        ...
        lin_vel_x_reward = -0.2 * torch.square(torch.relu(env.root_states[:, 7]))
        ...
        delta_gravity_x = env.projected_gravity[:, 0] - env.last_projected_gravity[:, 0]
        condition = env.projected_gravity[:, 2] < 0  # Corrected to use the z-axis as per your description
        ...
        total_reward = (roll_rate_reward + yaw_rate_reward + roll_reward + yaw_reward + pitch_rate_reward +
                        delta_pitch_reward + lin_vel_z_reward + lin_vel_x_reward + torques_reward + dof_acc_reward +
                        base_hit_ground_reward + gravity_reward + rotate_gravity_reward + height_history_reward)
