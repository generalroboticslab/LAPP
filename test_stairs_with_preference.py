...
task_registry.register('go2_stairs', LLMPrefGo2RobotStairs, LLMPrefGo2RobotStairsCfg, LLMPrefGo2RobotStairsCfgPPO)
...
def play(args):
    ...
    if args.terrain == 'pyramid_stairs':
        ...
        env_cfg.terrain.num_rows = 20  # 10
        ...
        #     'step_height': step_height,  # Set the step height [meters] 0.15
        #     'platform_size': 3.0  # Set the platform size [meters]  1.0, 1.5
        # }
        difficulty_level = 5 # 7
    ...
    out_fixed = cv2.VideoWriter(args.log_root + '/' + args.load_run + '/' + args.terrain + '_difficulty' + str(int(difficulty_level)) + '_model' + str(args.checkpoint) + '_' + args.test_direct + '_fixed.mp4', fourcc, 60.0, (camera_props.width, camera_props.height))
    print("save fixed video to:")
    print(args.log_root + '/' + args.load_run + '/' + args.terrain + '_difficulty' + str(int(difficulty_level)) + '_model' + str(args.checkpoint) + '_' + args.test_direct + 'fixed.mp4')
    ...
    out_dynamic = cv2.VideoWriter(args.log_root + '/' + args.load_run + '/' + args.terrain + '_difficulty' + str(int(difficulty_level)) + '_model' + str(args.checkpoint) + '_' + args.test_direct + '_follow.mp4', fourcc, 60.0, (camera_props.width, camera_props.height))
    print("save dynamic video to:")
    print(args.log_root + '/' + args.load_run + '/' + args.terrain + '_difficulty' + str(int(difficulty_level)) + '_model' + str(args.checkpoint) + '_' + args.test_direct + '_follow.mp4')
    ...
    env.max_episode_length = 400  # originally 1200
    ...
    for i in range(int(env.max_episode_length)):  # 10 * int(env.max_episode_length)
        ...
        print(env.base_lin_vel[0].cpu().numpy())
        ...
        if args.save_right_camera_figures and (i % 2 == 0) and (i < 600):
            success = cv2.imwrite(os.path.join("results_fig/right_video_figs", args.video_figures_path, f"right_{i}.png"), image_bgr_dynamic)

            # # image_bgr_dynamic must be H x W x 3, dtype=uint8
            # pil_image = Image.fromarray(image_bgr_dynamic[..., ::-1])  # if you need RGB, flip color channels
            # output_path = os.path.join("results_fig/right_video_figs", args.video_figures_path, f"right_{i}.png")
