...
from repo.unitree_rl_gym.legged_gym.utils import get_args, task_registry_backflip_pref
...
def main():
    ...
    env_ppo_cfg.runner.num_steps_per_env = args.num_steps_per_env  # 24, originally 128 or 72
    env_cfg.env.num_steps_per_env = args.num_steps_per_env

    # Create the environment
    # task_registry.register(args.task, LLMHydra2Go2JumpRobot, env_cfg, LLMHydra2Go2RobotCfgPPO)
    task_registry_backflip_pref.register(args.task, LLMPrefGo2LightBackflipRobot, env_cfg, env_ppo_cfg)
    # env, env_cfg = task_registry.make_env(name=args.task, args=args, env_cfg=env_cfg)
    env, env_cfg = task_registry_backflip_pref.make_env(name=args.task, args=args)

    # Proceed with training
    ppo_runner, train_cfg = task_registry_backflip_pref.make_alg_runner(
        env=env, name=args.task, args=args, log_root=args.log_root
    )

    print("running ppo_runner.preference_learn_train_pref_predictor3")
    ppo_runner.preference_learn_train_pref_predictor(
