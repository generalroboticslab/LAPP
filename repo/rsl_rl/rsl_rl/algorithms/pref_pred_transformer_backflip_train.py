...
# modified from PrefPredTransformerTrain10 class
# remove the commands from the state, since backflip does not rely on commands now
...
# mode 0: state(11), 1: obs(48), 2: state(11)+action(12), 3: obs(48)+action(12), 4: state(11)+obs(48)+action(12)
...
class PrefPredTransformerBackflipTrain:
    def __init__(self, org_data: dict, device, save_models_path, num_envs, save_models=True, pool_models_num=5, select_models_num=3, input_mode=0,
                 batch_size=256, transformer_embed_dim=64, seq_length=10, epsilon=0.1, lr=1e-3, weight_decay=1e-4, epochs=100):
        ...
        self.states_queue = torch.zeros((self.batch_size, self.seq_length, self.feature_dim)).float().to(self.device)
        self.predict_states_queue = torch.zeros((self.num_envs, self.seq_length, self.feature_dim)).float().to(self.device)

    def clear_states_queue(self):
        # clear the queues to zeros
        self.states_queue.zero_().to(self.device)

    def clear_predict_states_queue(self):
        # clear the queues to zeros
        self.predict_states_queue.zero_().to(self.device)
    ...
    def org_data_to_state_label_data(self, org_data: dict):
        ...
        state_tensor = torch.cat((base_lin_vel_tensor, base_ang_vel_tensor, base_height_tensor, rpy_tensor, feet_contacts_tensor), dim=3)
        obs_tensor = org_data['obs_pairs_buf']  # torch.Size([100, 2, 24, 48])
        action_tensor = org_data['actions_buf']  # torch.Size([100, 2, 24, 15])
        if self.input_mode == 0:
            input_tensor = state_tensor  # torch.Size([100, 2, 24, 15])
        elif self.input_mode == 1:
            input_tensor = obs_tensor  # torch.Size([100, 2, 24, 48])
        elif self.input_mode == 2:
            input_tensor = torch.cat((state_tensor, action_tensor), dim=3)  # torch.Size([100, 2, 24, 27])
        elif self.input_mode == 3:
            input_tensor = torch.cat((obs_tensor, action_tensor), dim=3)  # torch.Size([100, 2, 24, 60])
        elif self.input_mode == 4:
            input_tensor = torch.cat((state_tensor, obs_tensor, action_tensor), dim=3)  # torch.Size([100, 2, 24, 75])
        elif self.input_mode == 5:
            # this is used for gait pattern preference learning
            input_tensor = org_data['feet_contacts_buf'].float()  # torch.Size([100, 2, 24, 4])
        else:
    ...
    def predict_batch_sequence_reward(self, current_state):
        """
        Given a batch of current_state (shape [num_envs, features]),
        push the current state into a queue for prediction with shape [num_envs, seq_length, features],
        and then predict the reward.
        """
        if not self.models:
            raise ValueError("Models not trained. Call train() first.")
        # current_state has size [num_envs, feature_dim]
        # unsqueeze the current_state to have a seq_length dimension, and seq_length == 1
        current_state = current_state.unsqueeze(1).to(self.device)  # [num_envs, 1, feature_dim]

        self.predict_states_queue = torch.cat((self.predict_states_queue[:, 1:, :], current_state), dim=1)

        # Predict from each model
        rewards = []
        with torch.no_grad():
            for model in self.models:
                raw_rewards = model(self.predict_states_queue)  # [num_envs, seq_length, 1]
                raw_rewards = raw_rewards.squeeze()  # [num_envs, seq_length]
                raw_rewards = raw_rewards[:, -1]  # [num_envs, ]
