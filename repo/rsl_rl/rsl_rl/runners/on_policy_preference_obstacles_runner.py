...
from rsl_rl.algorithms import PrefPredTransformerObstaclesTrain
...
class OnPolicyPrefObstaclesRunner:
    ...
    def __init__(self,
        ...
        print(f"the save_interval for policy is: {self.save_interval}")
        ...
        if env.cfg.terrain.terrain_type == 'curriculum_obs':
            self.init_system_file_path = self.project_root / 'go2obstacles_prompt/obstacles_forward_initialize_system.txt'
        else:
        ...
        # self.pref_pred_models_root = self.project_root / 'logs/go2_wave_curr/Dec29_00-01-34_'
    ...
    def curr_learn_w_pred(self, num_learning_iterations, main_args, init_at_random_ep_len=False):
        ...
        self.base_pos_pref_train_buf = torch.zeros(main_args.num_pref_pairs_total_train, 2,
                                                   self.env.cfg.env.num_steps_per_env,
                                                   len(self.env.base_pos[1]), device=self.env.device,
                                                   dtype=torch.float,
                                                   requires_grad=False)
        self.rpy_pref_train_buf = torch.zeros(main_args.num_pref_pairs_total_train, 2,
                                              self.env.cfg.env.num_steps_per_env,
                                              len(self.env.rpy[1]),
                                              device=self.env.device, dtype=torch.float, requires_grad=False)
        self.base_lin_vel_pref_train_buf = torch.zeros(main_args.num_pref_pairs_total_train, 2,
                                                       self.env.cfg.env.num_steps_per_env,
                                                       len(self.env.base_lin_vel[1]), device=self.env.device,
                                                       dtype=torch.float,
                                                       requires_grad=False)
        self.base_ang_vel_pref_train_buf = torch.zeros(main_args.num_pref_pairs_total_train, 2,
                                                       self.env.cfg.env.num_steps_per_env,
                                                       len(self.env.base_ang_vel[1]), device=self.env.device,
                                                       dtype=torch.float,
                                                       requires_grad=False)
        self.feet_contacts_pref_train_buf = torch.zeros(main_args.num_pref_pairs_total_train, 2,
                                                        self.env.cfg.env.num_steps_per_env,
                                                        len(self.env.feet_indices), dtype=torch.bool,
                                                        device=self.env.device,
                                                        requires_grad=False)
        self.commands_pref_train_buf = torch.zeros(main_args.num_pref_pairs_total_train, 2,
                                                   self.env.cfg.env.num_steps_per_env,
                                                   self.env.cfg.commands.num_commands, device=self.env.device,
                                                   dtype=torch.float,
                                                   requires_grad=False)
        self.actions_pref_train_buf = torch.zeros(main_args.num_pref_pairs_total_train, 2,
                                                  self.env.cfg.env.num_steps_per_env,
                                                  self.env.num_actions,
                                                  device=self.env.device, dtype=torch.float, requires_grad=False)
        self.obs_pairs_pref_train_buf = torch.zeros(main_args.num_pref_pairs_total_train, 2,
                                                    self.env.cfg.env.num_steps_per_env,
        ...
        if main_args.save_pairs:
            ...
            initial_pref_dataset_selected_pairs_ids = self.select_random_pairs(traj_num=self.env.num_envs,
                                                                               pairs_num=main_args.num_pref_pairs_total_train)
                ...
                for i in range(self.num_steps_per_env):
                    ...
                    for k in range(len(initial_pref_dataset_selected_pairs_ids)):
                        ...
                        for first_or_second_in_pair in range(2):  # first_or_second_in_pair will be 0 or 1
                            self.base_pos_pref_train_buf[current_pair_id][first_or_second_in_pair][i][:] = \
                            self.env.base_pos[initial_pref_dataset_selected_pairs_ids[k][first_or_second_in_pair]][:]
                            # Subtract terrain height ==================
                            terrain_height = self.env._get_terrain_heights(
                                x=self.base_pos_pref_train_buf[current_pair_id][first_or_second_in_pair][i][0],
                                y=self.base_pos_pref_train_buf[current_pair_id][first_or_second_in_pair][i][1])
                            self.base_pos_pref_train_buf[current_pair_id][first_or_second_in_pair][i][
                                2] -= terrain_height.squeeze()
                            # ==================
                            self.rpy_pref_train_buf[current_pair_id][first_or_second_in_pair][i][:] = self.env.rpy[
                                                                                                          initial_pref_dataset_selected_pairs_ids[
                                                                                                              k][
                                                                                                              first_or_second_in_pair]][
                                                                                                      :]
                            self.base_lin_vel_pref_train_buf[current_pair_id][first_or_second_in_pair][i][:] = \
                            self.env.base_lin_vel[initial_pref_dataset_selected_pairs_ids[k][first_or_second_in_pair]][
                            :]
                            self.base_ang_vel_pref_train_buf[current_pair_id][first_or_second_in_pair][i][:] = \
                            self.env.base_ang_vel[initial_pref_dataset_selected_pairs_ids[k][first_or_second_in_pair]][
                            :]
                            self.feet_contacts_pref_train_buf[current_pair_id][first_or_second_in_pair][i][:] = \
                            self.env.contact_forces[initial_pref_dataset_selected_pairs_ids[k][
                                first_or_second_in_pair], self.env.feet_indices, 2] > 1.
                            self.commands_pref_train_buf[current_pair_id][first_or_second_in_pair][i][:] = \
                            self.env.commands[initial_pref_dataset_selected_pairs_ids[k][first_or_second_in_pair]][:]
                            # also save the actions in the trajectory
                            self.actions_pref_train_buf[current_pair_id][first_or_second_in_pair][i][:] = actions[
                                                                                                              initial_pref_dataset_selected_pairs_ids[
                                                                                                                  k][
                                                                                                                  first_or_second_in_pair]][
                                                                                                          :]
                            self.obs_pairs_pref_train_buf[current_pair_id][first_or_second_in_pair][i][:] = obs[
                                                                                                                initial_pref_dataset_selected_pairs_ids[
                                                                                                                    k][
                                                                                                                    first_or_second_in_pair]][
                                                                                                            :]
            ...
            torch.save(pref_train_data, os.path.join(self.log_dir, 'traj_pairs_initial{}.pt'.format(
                main_args.num_pref_pairs_total_train)))
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            if it % main_args.pref_pred_update_period_eps == 0:
                ...
                pref_pred_trainer = PrefPredTransformerObstaclesTrain(org_data=pref_train_data,
                                                              device=self.device,
