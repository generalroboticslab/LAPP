...
class CustomReward():
    ...
    def compute_reward(self):
        ...
        collision_penalty = -1. * torch.sum(1.0 * (torch.norm(env.contact_forces[:, env.penalised_contact_indices, :], dim=-1) > 0.13), dim=1)
        ...
        stumble_penalty = -10.0 * torch.sum(stumble, dim=1)
    ...
    def compute_success(self):
        ...
        # 0.0035 -> 0.0025
        collision_penalty = -0.8 * torch.sum(1.0 * (torch.norm(env.contact_forces[:, env.penalised_contact_indices, :], dim=-1) > 0.13), dim=1)
